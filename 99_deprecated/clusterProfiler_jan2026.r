...
analyze_comparison <- function(cell_types, working_base, mapped_data_base, organism, ont, 
  ...
  if (!is.null(gse_for_plot) && nrow(gse_for_plot@result) > 0) {
    ...
    save_plot_organized(cnetplot(gse_for_plot, categorySize = "geneNum", foldChange = gene_list), 
  ...
  save_plot_organized(cnetplot(kk2, categorySize = "geneNum", foldChange = gene_list), "KEGG_cnet.svg", dirs$plots_kegg)
  ...
  save_plot_organized(cnetplot(gsea_kegg_selected, categorySize = "geneNum", foldChange = selected_kegg_list), "KEGG_Predefined_cnet.svg", dirs$plots_kegg)
