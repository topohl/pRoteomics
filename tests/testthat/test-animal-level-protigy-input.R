...
testthat::test_that("neuropil regions and layers remain separate", {
  ...
  testthat::expect_identical(
    ...
    c("id", "A0001_CA1_slm", "A0001_CA1_sr", "A0001_CA2_slm")
  )
  testthat::expect_equal(result$animal_expression$A0001_CA1_slm[[1]], 2)
...
testthat::test_that("strict bilateral sensitivity differs only by one-sided units", {
  ...
  testthat::expect_identical(
    names(result$animal_expression),
    c("id", "A0001_CA1_slm", "A0002_CA1_slm")
  )
  testthat::expect_identical(
...
testthat::test_that("strict GCT v1.3 dimensions match animal-level contents", {
  ...
  testthat::expect_identical(
    dims,
    c(result$n_proteins, nrow(result$output_metadata), 0L, ncol(result$output_metadata))
  )
...
testthat::test_that("cmapR dimensions exclude reserved id with zero row descriptors", {
  metadata <- data.frame(
  ...
  testthat::expect_identical(lines[[2]], "2\t3\t0\t2")
  testthat::expect_identical(lengths(lapply(lines[3:7], gct_v1.3_fields)), rep(4L, 5L))
  testthat::expect_identical(
    lines[3:7],
    c(
      "id\ts1\ts2\ts3",
...
testthat::test_that("02_excel_convert shared writer uses cmapR-compatible id semantics", {
  ...
  testthat::expect_identical(names(prepared), c("id", "sample1"))
  ...
  testthat::expect_identical(names(alternate_prepared), c("id", "sample1"))
