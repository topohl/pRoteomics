...
testthat::test_that("DA statistics are joined with provenance, not recomputed", {
  ...
  src <- paste(readLines(repo_path("analysis/spatial_validation",
                                   "build_protein_spatial_atlas.R"),
                         warn = FALSE), collapse = "\n")
...
testthat::test_that("labels and WGCNA membership are unchanged by the atlas", {
  ...
  for (f in c("build_module_spatial_atlas.R", "build_protein_spatial_atlas.R",
              "quantify_neuropil_detection_context.R")) {
    src <- readLines(repo_path("analysis/spatial_validation", f), warn = FALSE)
