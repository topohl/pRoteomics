...
testthat::test_that("WGCNA separates quantitative inclusion from annotation eligibility", {
  script <- paste(readLines(repo_path("analysis/wgcna", "build_wgcna_modules.R"), warn = FALSE), collapse = "\n")
...
testthat::test_that("MapThatProt and WGCNA source shared protein mapping utilities", {
  mapthatprot <- paste(readLines(repo_path("analysis/preprocessing", "map_protein_identifiers.R"), warn = FALSE), collapse = "\n")
  wgcna <- paste(readLines(repo_path("analysis/wgcna", "build_wgcna_modules.R"), warn = FALSE), collapse = "\n")
