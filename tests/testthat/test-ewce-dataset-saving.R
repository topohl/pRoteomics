...
testthat::test_that("EWCE script parses --dataset before output paths are created", {
  txt <- paste(readLines(repo_path("analysis/enrichment/run_ewce_celltype_enrichment.R"), warn = FALSE), collapse = "\n")
...
testthat::test_that("EWCE dry-run honors dataset-specific output folders", {
  run <- function(dataset) {
    ...
    out <- suppressWarnings(system2(
      ...
      c("analysis/enrichment/run_ewce_celltype_enrichment.R", "--dataset", dataset, "--dry-run"),
      stdout = TRUE,
      stderr = TRUE
    ))
    paste(out, collapse = "\n")
...
testthat::test_that("EWCE animal mode is isolated and reports the shared aggregation contract", {
  ...
  out <- suppressWarnings(system2(
    file.path(R.home("bin"), "Rscript"),
    c("analysis/enrichment/run_ewce_celltype_enrichment.R", "--dataset", "neuron_soma", "--dry-run"),
    stdout = TRUE,
    stderr = TRUE
  ))
...
testthat::test_that("EWCE animal-level safeguards and cache identity are present", {
  script <- repo_path("analysis/enrichment/run_ewce_celltype_enrichment.R")
...
testthat::test_that("EWCE signature-only mode exits before parallel or bootstrap execution", {
  txt <- paste(
    readLines(repo_path("analysis/enrichment/run_ewce_celltype_enrichment.R"), warn = FALSE),
    collapse = "\n"
  )
