...
testthat::test_that("Stage 13 is a parseable non-circular microglia handoff", {
  stage13 <- file.path(root, "analysis/wgcna", "audit_module_claim_readiness.R")
  ...
  for (script in c("test_module_phenotypes.R", "annotate_module_microenvironment.R", "summarize_module_interpretation.R", "audit_microglia_module_claims.R")) {
    source_text <- paste(readLines(file.path(root, "analysis/wgcna", script), warn = FALSE), collapse = "\n")
...
testthat::test_that("future cut-height defaults remain distinct from frozen microglia provenance", {
  stage01 <- file.path(root, "analysis/wgcna", "build_wgcna_modules.R")
  ...
  stage12_text <- paste(readLines(file.path(root, "analysis/wgcna", "audit_microglia_module_claims.R"), warn = FALSE), collapse = "\n")
