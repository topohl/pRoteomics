...
testthat::test_that("publication entrypoint is rendering-only and output-isolated", {
  ...
  script <- paste(readLines(file.path(root, "analysis/qc", "render_joint_compartment_qc_figures.R"), warn = FALSE), collapse = "\n")
  ...
  testthat::expect_match(registry, "analysis/qc/render_joint_compartment_qc_figures.R", fixed = TRUE)
})

testthat::test_that("anatomical-island PCA has a panel-local tightened PC2 display range", {
  root <- testthat::test_path("..", "..")
  script <- paste(readLines(file.path(
    root, "analysis/qc", "render_joint_compartment_qc_figures.R"
  ), warn = FALSE), collapse = "\n")
