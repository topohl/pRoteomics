...
testthat::test_that("pipeline manuscript entry points declare authoring outputs only", {
  ...
  # Inventory guard: figure_01 and its panel producer, figure_02, figure_03,
  # and the two manuscript-supporting immunostaining renderers.
  # Bump deliberately when a figure entry point is added, so an accidental one
  # is still caught.
  testthat::expect_length(figure_steps, 6L)
