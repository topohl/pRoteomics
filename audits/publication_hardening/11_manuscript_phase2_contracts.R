...
f1 <- rbind(
  ...
  F1("CombZ / outcome-score construction", "NOT IN REPOSITORY",
     "read at analysis/01_preprocessing/06_merged_metadata_module_score.r:399-427",
  ...
  F1("RES / SUS classification rule", "NOT IN REPOSITORY",
     "analysis/01_preprocessing/06_merged_metadata_module_score.r StressGroup = case_when(!is.na(group_auc_all) ~ group_auc_all, ...)",
...
mp <- rbind(
  ...
  M("M-04", "Ranked GO enrichment",
    ...
    "analysis/04_differential_abundance/01_clusterProfiler.r", "gseGO call",
