#!/usr/bin/env Rscript
# ================================================================
# Script: analysis/05_wgcna/summarize_module_scores.R
...
# Notes: Final publication rendering for score-derived supermodule robustness/correlation plots; statistics remain produced by score_module_activity.R.
...
if (!module_source_arg %in% c("wgcna")) {
  stop("summarize_module_scores.R currently renders WGCNA-source supermodule score outputs only.", call. = FALSE)
}
...
run_one_dataset <- function(dataset, module_source = "wgcna") {
  ...
  if (run$dry_run) {
    dry_run_line("Script", "analysis/05_wgcna/summarize_module_scores.R")
  ...
  write_run_manifest(
    ...
    notes = "Final score-derived WGCNA supermodule publication plots. Score/statistics computation stays in score_module_activity.R; every rendered label comes only from the canonical lookup produced by 07."
  )
