...
# Script: analysis/05_wgcna/audit_microglia_module_claims.R
# Stage: modules_downstream
# Scope: dataset_specific
# Consumes: required data/processed/06_modules_WGCNA/01_WGCNA/microglia/wgcna_final_model_state.rds; results/tables/06_modules_WGCNA/group_effects/microglia/; optional results/tables/06_modules_WGCNA/microglia_neuropil_independence/microglia/; results/tables/06_modules_WGCNA/module_robustness_sensitivity/microglia/
# Produces: results/reviewer_audit/microglia_wgcna_nature_readiness/WGCNA_nature_readiness_report.md; results/reviewer_audit/microglia_wgcna_nature_readiness/module_robustness_consensus.csv; results/reviewer_audit/microglia_wgcna_nature_readiness/higher_order_block_readiness_summary.csv
# Dataset behavior: runs for microglia according to pipeline.yml and --dataset/PROTEOMICS_DATASET where supported.
# Notes: Additive, fixed-membership robustness audit for the current microglia WGCNA network.
...
cut_height_provenance <- rbind(
  ...
  data.frame(evidence_source = "analysis/05_wgcna/build_wgcna_modules.R", recorded_cut_height = configured_default_cut_height, evidence_type = "configured_future_default", authoritative = FALSE, consistent_with_current_membership = is.finite(state_cut) && isTRUE(all.equal(configured_default_cut_height, state_cut)), interpretation = "The intended future default is distinct from the selected value that generated the frozen historical network.", source_hash = source_hash(file.path(repo_root, "analysis/05_wgcna/build_wgcna_modules.R")), stringsAsFactors = FALSE)
)
...
report_lines <- c(
  ...
  "## Reproducibility",
  "",
  paste0("Audit script: `analysis/05_wgcna/audit_microglia_module_claims.R`; conventional preservation permutations: ", n_permutations, "; AnimalID-cluster bootstrap: ", n_animal_bootstrap, "; generated: ", format(Sys.time(), tz = "Europe/Berlin", usetz = TRUE), ". See `input_hashes.csv`, `protected_output_hash_audit.csv`, `validation_table.csv` and `session_info.txt`." )
)
...
readme_lines <- c(
  ...
  "",
  "```powershell",
  "Rscript .\\06_modules_WGCNA\\audit_microglia_module_claims.R --permutations 100 --animal-bootstrap 500",
  "Rscript .\\06_modules_WGCNA\\summarize_microglia_module_claims.R",
