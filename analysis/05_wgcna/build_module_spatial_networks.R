...
#   - dataset/source-scoped module score output from analysis/05_wgcna/score_module_activity.R
# Produces:
...
#   2) module_scores_per_sample.csv from score_module_activity.R
#
# Outputs:
...
if (!nzchar(dataset_arg)) {
  stop("build_module_spatial_networks.R requires --dataset or PROTEOMICS_DATASET.", call. = FALSE)
}
if (!nzchar(module_source_arg)) {
  stop("build_module_spatial_networks.R requires --module-definition-source or PROTEOMICS_MODULE_DEFINITION_SOURCE.", call. = FALSE)
}
...
if (is_dry_run()) {
  dry_run_line("Script", "analysis/05_wgcna/build_module_spatial_networks.R")
...
if (!file.exists(params$module_score_file) && !allow_regex_fallback) {
  stop("Module score output not found: ", params$module_score_file,
       ". Run analysis/05_wgcna/score_module_activity.R for dataset=", dataset_profile,
...
if (file.exists(params$module_score_file)) {
  message2("Reading module score output from score_module_activity.R: ", params$module_score_file)
...
write_run_manifest(
  ...
  notes = "Consumes dataset/source-scoped module scores from score_module_activity.R."
)
