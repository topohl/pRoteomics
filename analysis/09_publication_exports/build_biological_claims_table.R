#!/usr/bin/env Rscript
# ================================================================
# Script: analysis/09_publication_exports/build_biological_claims_table.R
# Stage: export
# Scope: global
...
SCRIPT_ID <- "analysis/09_publication_exports/build_biological_claims_table.R"
...
final_reviewer_audit_specs <- function() {
  ...
  tibble::tribble(
    ...
    "wgcna_label_confidence_audit.csv", file.path(audit_dir, "wgcna_label_confidence_audit.csv"), NA_character_, "analysis/05_wgcna/annotate_module_microenvironment.R", "WGCNA label confidence", FALSE, "Audit summary only.",
    "wgcna_annotation_source_audit.csv", file.path(audit_dir, "wgcna_annotation_source_audit.csv"), NA_character_, "analysis/05_wgcna/annotate_module_microenvironment.R", "WGCNA annotation provenance", FALSE, "Audit summary only.",
    "wgcna_microenvironment_threshold_sensitivity.csv", file.path(audit_dir, "wgcna_microenvironment_threshold_sensitivity.csv"), NA_character_, "analysis/05_wgcna/annotate_module_microenvironment.R", "Threshold sensitivity", FALSE, "Diagnostic audit only.",
    "microglia_neuropil_independence_claim_gate.csv", file.path(audit_dir, "microglia_neuropil_independence_claim_gate.csv"), "microglia_neuropil_independence_claim_gate", "analysis/05_wgcna/test_microglia_neuropil_independence.R", "Direct module-level independence claim gate", FALSE, "Module endpoints only unless endpoint_scope explicitly says supermodule.",
    "microglia_neuropil_covariate_selection_audit.csv", file.path(audit_dir, "microglia_neuropil_covariate_selection_audit.csv"), "microglia_neuropil_covariate_selection_audit", "analysis/05_wgcna/test_microglia_neuropil_independence.R", "Covariate selection provenance", FALSE, "Exploratory rows are diagnostic only.",
    "microglia_neuropil_independence_endpoint_scope_audit.csv", file.path(audit_dir, "microglia_neuropil_independence_endpoint_scope_audit.csv"), "microglia_neuropil_independence_endpoint_scope_audit", "analysis/05_wgcna/test_microglia_neuropil_independence.R", "Endpoint-scope certification", FALSE, "Supermodule claims require a direct supermodule endpoint.",
...
collect_program_claims <- function(dataset) {
  f <- resolve_input_path(
    ...
    dataset = dataset,
    stage = "export",
    producer_script_or_artifact_id = "analysis/04_differential_abundance/summarize_biological_programs.R"
  )
...
collect_wgcna_claims <- function(dataset) {
  f <- resolve_input_path(
    ...
    dataset = dataset,
    stage = "export",
    producer_script_or_artifact_id = "analysis/05_wgcna/build_wgcna_modules.R"
  )
  df <- read_csv_if_exists(f)
...
collect_overlap_claims <- function(dataset) {
  f <- resolve_input_path(
    ...
    dataset = dataset,
    stage = "export",
    producer_script_or_artifact_id = "analysis/05_wgcna/compare_module_enrichment_overlap.R"
  )
  df <- read_csv_if_exists(f)
  if (is.null(df) || !nrow(df) || !"ModuleID" %in% names(df)) return(empty_claims())
...
collect_behavior_claims <- function() {
  f <- resolve_input_path(
    ...
    dataset = "global",
    stage = "export",
    producer_script_or_artifact_id = "analysis/08_integration/test_module_behaviour_coupling.R"
  )
  df <- read_csv_if_exists(f)
  if (is.null(df) || !nrow(df)) return(empty_claims())
...
collect_microglia_signature_claims <- function(dataset) {
  f <- resolve_input_path(
    ...
    dataset = dataset,
    stage = "export",
    producer_script_or_artifact_id = "analysis/04_differential_abundance/test_microglia_targeted_signatures.R"
  )
  df <- read_csv_if_exists(f)
...
collect_wgcna_group_effect_claims <- function(dataset, level = c("module", "supermodule")) {
  ...
  f <- resolve_input_path(
    ...
    dataset = dataset,
    stage = "export",
    producer_script_or_artifact_id = "analysis/05_wgcna/summarize_module_interpretation.R"
  )
...
collect_integration_claims <- function() {
  f <- resolve_input_path(
    ...
    dataset = "global",
    stage = "export",
    producer_script_or_artifact_id = "analysis/08_integration/summarize_programs_for_manuscript.R"
  )
  df <- read_csv_if_exists(f)
...
if (is_dry_run()) {
  ...
  dry_run_line("Script", "analysis/09_publication_exports/build_biological_claims_table.R")
