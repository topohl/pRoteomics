  ...
  expected |>
    dplyr::mutate(
      ...
      recommended_action = dplyr::case_when(
        ...
        grepl("01_WGCNA", .data$producing_script) ~ "run Rscript analysis/wgcna/build_wgcna_modules.R --dataset neuron_neuropil with cached state reuse enabled",
        grepl("06_annotate", .data$producing_script) ~ "run Rscript analysis/wgcna/annotate_module_microenvironment.R --dataset neuron_neuropil after 01_WGCNA/group effects exist",
        grepl("07_wgcna", .data$producing_script) ~ "run Rscript analysis/wgcna/summarize_module_interpretation.R --dataset neuron_neuropil after existing Stage 05 group effects and annotation exist",
  ...
  rows <- lapply(datasets, function(ds) {
    ...
    dplyr::bind_rows(
      ...
      source_summary_row(
        ...
        "Cohen_d", "Analysis", "within_BH_significant/p_adj_within_model_BH", "within_BH_significant == TRUE or p_adj_within_model_BH <= 0.05",
        "Primary table consumed by analysis/wgcna/summarize_module_scores.R."
      ),
      source_summary_row(
        ...
        "Cohen_d", "Analysis", "within_BH_significant/p_adj_within_model_BH", "within_BH_significant == TRUE or p_adj_within_model_BH <= 0.05",
        "Source data written by analysis/wgcna/summarize_module_scores.R."
      ),
      source_summary_row(
...
if (run$dry_run) {
  ...
  dry_run_line("Script", "analysis/integration/render_module_circular_atlas.R")
