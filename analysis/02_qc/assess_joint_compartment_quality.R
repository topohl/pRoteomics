#!/usr/bin/env Rscript
# Script: analysis/02_qc/assess_joint_compartment_quality.R
# Stage: qc_global
# Scope: global
...
runtime <- list(script = "analysis/02_qc/assess_joint_compartment_quality.R", stage = "qc_global", dataset = "global", args = commandArgs(trailingOnly = TRUE), dry_run = is_dry_run(), started_at = Sys.time())
...
if (!file.exists(bundle_file)) stop("Joint preprocessing bundle is missing: ", bundle_file, ". Run build_joint_protigy_input.R first.", call. = FALSE)
