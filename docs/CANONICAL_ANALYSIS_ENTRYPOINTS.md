...
|---|---|---|
| Joint Protigy input, animal-level matrices | `analysis/preprocessing/build_joint_protigy_input.R` | joint_qc_preprocessing |
| GCT extraction, merged metadata | `analysis/preprocessing/extract_protigy_contrasts.R`, `analysis/preprocessing/build_module_score_metadata.R` | core |
| Protein ID mapping | `analysis/preprocessing/map_protein_identifiers.R` | core |
| Per-dataset QC report | `analysis/qc/assess_dataset_quality.R` | qc |
| Cross-compartment QC | `analysis/qc/assess_joint_compartment_quality.R` | qc_global |
| Marker detectability / WGCNA bridge | `analysis/qc/summarize_marker_detectability.R` | qc |
| **Canonical GSEA (GO-BP)** | `analysis/differential_abundance/run_clusterprofiler_enrichment.R` | enrichment |
| Cross-contrast GO comparison | `analysis/differential_abundance/compare_go_enrichment.R` | enrichment |
| **Spatial GO-program atlas** | `analysis/differential_abundance/build_go_program_atlas.R` | enrichment |
| SUS/RES spatial DAP atlas | `analysis/differential_abundance/build_sus_res_dap_atlas.R` | enrichment |
| **External signature validation** | `analysis/differential_abundance/validate_control_spatial_identity.R` | enrichment |
| Cell-type enrichment (EWCE) | `analysis/enrichment/run_ewce_celltype_enrichment.R` | enrichment |
| **WGCNA modules** | `analysis/wgcna/build_wgcna_modules.R` | modules_wgcna |
| WGCNA identity contract | `analysis/wgcna/build_module_identity_contract.R` | modules_downstream |
| WGCNA label adjudication | `analysis/wgcna/adjudicate_module_labels.R` | networks |
| Spatial networks | `analysis/spatial_networks/build_spatial_networks.R` | networks |
| **Animal-level spatial networks** | `analysis/spatial_validation/build_animal_spatial_networks.R` | networks |
| Bilateral spatial identity | `analysis/spatial_validation/quantify_bilateral_spatial_identity.R` | networks |
| CA2-SLM robustness | `analysis/spatial_validation/audit_ca2_slm_robustness.R` | networks |
| Behaviour coupling | `analysis/integration/test_behaviour_proteomics_associations.R` | coupling |
| Edge-behaviour coupling | `analysis/integration/test_network_behaviour_coupling.R` | coupling |
| **GSEA × WGCNA concordance** | `analysis/integration/test_enrichment_module_concordance.R` | integration |
| Cross-compartment program atlas | `analysis/integration/build_cross_compartment_atlas.R` | integration |
| Evidence priority matrix | `analysis/integration/build_evidence_priority_matrix.R` | integration |
| PRIDE / journal export | `analysis/publication_source_data/RUN_EXPORT.R` | export (wrapper) |
| Supplementary tables | `analysis/publication_source_data/build_supplementary_tables.R` | export |
| Source data release | `analysis/publication_source_data/09_export_source_data.R` | export |
...
| clusterProfiler enrichment | `analysis/differential_abundance/run_clusterprofiler_enrichment.R` | the eleven `90_testing/clusterProfiler*` scratch scripts and four `99_deprecated/clusterProfiler*` files |
...
this repository owns is the source data: `analysis/publication_source_data/` and
