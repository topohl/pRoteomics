...
| clusterProfiler executed ranked order | `protected_reference_not_consumed`, declared by `clusterprofiler_protected_reference_artifacts()` in `R/enrichment/enrichment_io.R`. Written from `names(gene_inputs$sensitivity$median)`, the ordered vector passed to `gseGO()`, so it records the rank order that executed rather than an input it could be recomputed from (Phase 6I.2: `BYTE_EXACT_STORED_ORDER`, 54/54 comparisons). The clusterProfiler manifest has no column for it and nothing reads it, which is why no registry previously knew it existed. Not cleanup-eligible: computationally reconstructible is not disposable provenance, and this is the primary record behind the Figure 3 g/h/i selection. | per-comparison `protein_group_audits/rank_statistic_sensitivity_audit.csv`, a sibling of the manifest-declared `collapsed_gene_input_file` (54 instances; 18 sit past the 260-character wall and resolve only through the Phase 6H.3 staging contract) |
...
| Figure 2 control spatial identity validation | `control_spatial_identity_validation_v1`; CON-only limma repeated-measures anatomy contrasts, unchanged ProteinGroupID results and canonical official-gene collapse | `results/tables/04_differential_expression_enrichment/control_spatial_identity_validation/global/{anatomical_protein_contrasts,kaulich_signature_mapping,kaulich_signature_gsea,control_anatomical_go_bp_gsea,analysis_status}.csv`; matching Figure 2e/2f source CSVs, SVGs, and `results/logs/.../run_manifest.yml` |
...
| SUS-RES manuscript GO-theme registry and supported-term audit | GO-ID anchors plus GO.db `is_a`/`part_of` paths; every FDR-supported occurrence retained, including unclassified and QC-review terms | `config/manuscript_go_theme_registry.tsv`; `results/source_data/04_differential_expression_enrichment/compareGO_spatial_atlas/sus_res_{supported_go_term_theme_audit,go_term_theme_assignment_long,go_semantic_redundancy_audit}.csv` |
| SUS-RES biological inspection summary | downstream-only ontology-mapped theme × spatial-unit grid; descriptive median/Q25/Q75 NES use all mapped tested GO-BP terms, while original constituent GO-term BH FDR support and representative supported terms remain separate; no theme-level p-value/FDR | `results/tables/04_differential_expression_enrichment/sus_res_spatial_dap_atlas/global/sus_res_manuscript_theme_summary.csv`; all-term machine source under matching `results/source_data/...` |
| SUS-RES biological audit workbook | human-facing view of the canonical CSV contracts with Overview, Spatial Map, Theme Detail, GO Term Audit, DAP Summary, and Provenance sheets; not a computational input | `results/reports/04_differential_expression_enrichment/sus_res_spatial_dap_atlas/global/sus_res_biological_audit.xlsx` |
| Three-contrast stress-response biological audit | downstream matched ProteinGroupID geometry and all-three-contrast GO-theme trajectories. Original per-contrast protein/GO BH FDR remains authoritative. Secondary control-pair BH is applied only to `RES_vs_CON` and `SUS_vs_CON` raw nominal p-values within each dataset x spatial unit matched ProteinGroupID or GO-ID universe; `SUS_vs_RES` remains outside the joint families. Direct-DAP control-reference geometry starts only from canonical `SUS_vs_RES` FDR < 0.05 and is a descriptive decomposition, not independent control-contrast validation. Median absolute log2FC, farther-from-control fractions, median theme NES, theme delta-magnitude summaries, and trajectory geometry are descriptive with no new p-value. FDR-supported GO occurrences have a unique `dataset | spatial_unit | phenotype_contrast | ID` key before explicit occurrence x theme-assignment expansion. Legacy script-07 heuristic classes are not manuscript evidence. | `results/{tables,source_data,figures,reports,logs}/04_differential_expression_enrichment/stress_response_biological_audit/global/` |
| Stage-11 protected-reference provenance | deterministic record of protected artifacts that Stage 11 hashes for reference but does not consume or modify. The SUS-RES audit workbook is not a computational input. | `results/source_data/04_differential_expression_enrichment/stress_response_biological_audit/global/protected_reference_artifacts.csv` |
| Stage-11 external protected-hash validation | post-run reviewer audit kept outside the Stage-11 producer outputs, with validation HEAD/timestamp/role and expected/observed SHA-256 values. A comparison is current evidence only for the recorded validation HEAD and timestamp. | `results/reviewer_audit/stress_response_biological_audit/protected_hash_comparison.csv` |
...
| Control compartment marker abundance/detection | `control_compartment_abundance_v2`; observed non-imputed, CON-only, animal-level descriptive contract | `results/source_data/03_qc_exploration/04e_control_compartment_abundance_publication_figures/global/v2_00_joint_normalization_reconstruction_audit.csv` through `v2_18c_terminal_rank_drop_audit.csv`; v2 run/render manifests under `results/logs/.../global/`; v2 interpretation/legends under `results/reports/.../global/`; detection-aware dot heatmap and rank-abundance figures under `results/figures/.../global/`. Primary keys and row meanings are file-specific and validated before writing. Every CSV records observed/imputed status, normalization source, feature universe, animal/region thresholds, hemisphere handling, marker-source policy, and selection policy. The historical v1 files and `control_compartment_publication_labels.csv` are legacy-only and are not overwritten or consumed by v2. |
...
| WGCNA module/supermodule group effects | `wgcna_group_effects_phase2b_v5`; `wgcna_group_validate_output_bundle()`; equal-weight left/right animal-spatial-unit ML inference after within-hemisphere technical-row means; cross-platform `sha256_utf8_lf_v1` aggregate hashes; variance-ratio boundary classification with separate `lme4::isSingular()` diagnostic; explicit single-fit, full-interaction, or composite reduced/full diagnostic scope; disjoint tier-specific BH families; singleton aliases have inherited statistics and diagnostics but no FDR; Stage 05 readiness deliberately unassessed | `results/tables/06_modules_WGCNA/group_effects/<dataset>/module_group_effects.csv`; `supermodule_group_effects.csv`; `supermodule_composition.csv`; `WGCNA_group_effect_endpoint_provenance.csv`; `WGCNA_group_effect_model_validation.csv`; `WGCNA_group_effect_sample_inclusion_audit.csv`; `WGCNA_group_effect_animal_spatial_unit_values.csv`; `WGCNA_group_effect_hemisphere_values.csv`; `WGCNA_group_effect_interaction_conditional_followup.csv`; `WGCNA_group_effect_sensitivity.csv`; `WGCNA_group_effect_left_right_concordance.csv`; `WGCNA_group_effect_downstream_consumer_migration_audit.csv`; `WGCNA_group_effect_contract_status.csv`; PCA/value audits; `WGCNA_group_effect_legacy_output_staleness_audit.csv`; source-data mirrors and run logs. Existing generated migration-status files predate the completed Phase 3 source migration and remain stale until a later authorized output refresh; claim-facing consumers use only the Stage 07 inferential handoff. |
...
| WGCNA interpretable summary and inferential handoff | `wgcna_inferential_handoff.yml`; existing Phase 2B consumer adapter; automatic proposals in `wgcna_label_candidates.yml`; reviewed microglia lookup in `wgcna_reviewed_final_label_lookup.yml` | `results/tables/06_modules_WGCNA/interpretable_summary/<dataset>/WGCNA_inferential_handoff.csv` is the sole claim-facing inferential handoff. It contains every independent module endpoint plus genuine supermodule endpoints with at least two member modules, uses adapter-copied tier-specific FDR and family metadata, carries `model_valid_for_inference` and `claim_gate`, and traces each row to the exact Stage 05 `source_artifact` and `source_key`. Singleton-supermodule aliases remain in `WGCNA_final_label_lookup.csv` but are excluded from independent endpoints. Full compatibility/interpretable outputs remain in `WGCNA_{module,supermodule}_group_effects_interpretable.csv`; conditional interaction follow-ups remain separate diagnostics in `WGCNA_interaction_conditional_followup_interpretable.csv`; cross-dataset summary remains under `interpretable_summary/all/`. |
...
| microglia WGCNA Stage 13 claim readiness | `wgcna_entity_claim_readiness.yml` and strict shared loader | `results/tables/06_modules_WGCNA/claim_readiness/microglia/WGCNA_entity_claim_readiness.csv` |
...
In `biological_claims_table.csv`, `WGCNA_module_group_effect` and `WGCNA_supermodule_group_effect` rows are direct group-effect model rows and retain their Stage 05 statistics. `WGCNA_module` rows from `WGCNA_module_evidence_rank.csv` are architecture candidates, not group-effect tests. For microglia, all WGCNA claim rows use exact Stage 13 `dataset + level + entity_id` joins: aliases never create claim rows, SM01/SM03 are not independent entities, SM09 is the sole higher-order architecture candidate, and `group_effect_status = not_FDR_supported` blocks stress-effect claims without blocking architecture context.
