...
| `WGCNA_modules_long` | `results/tables/06_modules_WGCNA/01_WGCNA/<dataset>/modules/` plus `data/processed/06_modules_WGCNA/01_WGCNA/<dataset>/wgcna_final_model_state.rds` | Color-stable WGCNA module definitions are reusable inputs for dataset-scoped `03_score_module_activity.R` when `PROTEOMICS_MODULE_DEFINITION_SOURCE=WGCNA`; GO/manual labels feed `ModuleDisplayLabel` for figures, while `ModuleID` remains the join key. WGCNA-source scoring also emits secondary supermodule eigengene score/robustness outputs. `neuron_neuropil` keeps the curated-overlap score fallback and also has a WGCNA source score pass in the default pipeline. |
