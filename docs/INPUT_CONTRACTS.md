...
| mapped contrast | `mapped_contrast.yml` | `ProteinGroupID`, `original_identifier`, `member_accessions`, `member_gene_symbols`, `protein_group_ambiguity_class`, `gene_level_claim_allowed`, `protein_level_claim_allowed`, `mapping_status` | `data/processed/02_id_mapping/mapped/<dataset>/forward/per_file/` | Canonical Phase 1A mapped contrast. One quantitative input row remains one output protein-group row; all protein-group members are retained in `data/processed/02_id_mapping/member_bridge/<dataset>/forward/per_file/`. Legacy `gene_symbol` is display-only and deprecated as a quantitative key. Optional p-value columns `pvalue`, `pval`, `P.Value`, `padj`, and `adj.P.Val` must be in `[0, 1]` when present. |
