...
# ================== PCA Analysis and Plotting (Improved Folder & Naming Structure) ==================

# ---- 1. Setup & Packages ----
options(repos = c(CRAN = "https://cloud.r-project.org"))
if (!requireNamespace("pacman", quietly = TRUE)) install.packages("pacman")
pacman::p_load(
    data.table, ggplot2, factoextra, reshape2, stats, ggrepel, tools, grid, uwot, RColorBrewer, pheatmap, Rtsne, aricode, rospca, irlba, pandoc, treemapify, dplyr
)
set.seed(42)
...
# ---- 3. Folder Structure ----
folders <- list(
    meta         = file.path(output_root, "meta"),
    tables       = file.path(output_root, "tables"),
    plots        = file.path(output_root, "plots"),
    plots_base   = file.path(output_root, "plots", "base"),
    plots_var    = file.path(output_root, "plots", "variance"),
    plots_umap   = file.path(output_root, "plots", "umap"),
    plots_tsne   = file.path(output_root, "plots", "tsne"),
    plots_heat   = file.path(output_root, "plots", "heatmaps"),
    plots_box    = file.path(output_root, "plots", "boxplots"),
...
make_filename <- function(prefix, group = NULL, ext = "svg") {
    if (!is.null(group)) paste0(prefix, "_by_", group, ".", ext) else paste0(prefix, ".", ext)
}
...
mat <- g$mat
meta <- g$meta
if (!"sample_id" %in% colnames(meta)) stop("meta must have a sample_id column")
colnames(mat) <- trim_ws(colnames(mat))
meta$sample_id <- trim_ws(meta$sample_id)
rownames(meta) <- meta$sample_id
mat <- mat[, meta$sample_id, drop = FALSE]
if ("exclude" %in% colnames(meta)) {
    keep <- !tolower(meta$exclude) %in% c("true", "1", "yes")
...
mat <- mat[which(vz > 0 & is.finite(vz)), , drop = FALSE]
...
rownames(pca$x) <- colnames(mat)
meta <- meta[rownames(pca$x), , drop = FALSE]
stopifnot(identical(rownames(meta), rownames(pca$x)))

# ---- 8. Palette & Theme ----
base_hex <- c("#1E90FF", "#61d7ffff", "#FFA500", "#FFD700", "#FF4500", "#FF6347", "#FFDAB9", "#FFE4B5", "#F0E68C", "#ADFF2F", "#98FB98", "#FF69B4", "#FFB6C1", "#FF1493", "#FF4500")
...
theme_pca_min <- function() {
    theme_minimal(base_size = 16, base_family = "sans") +
        theme(
            ...
            axis.title = element_text(color = "#444444", size = 20, face = "plain"),
            axis.text  = element_text(color = "#555555", size = 20),
            ...
            legend.title = element_text(color = "#444444", size = 14),
            legend.text  = element_text(color = "#666666", size = 14),
            plot.title   = element_text(color = "#222222", face = "bold", size = 20, margin = margin(b = 6)),
            plot.subtitle= element_text(color = "#666666", size = 14, margin = margin(b = 6)),
            plot.caption = element_text(color = "#999999", size = 12),
            strip.text = element_text(color = "#333333", size = 14, face = "bold"),
...
build_group <- function(key) {
    if (!key %in% colnames(meta)) stop(sprintf("Key '%s' not in meta.", key))
    v <- trim_ws(as.character(meta[[key]]))
    v[v == ""] <- NA
    factor(v)
}

# ---- 10. Example Plotting (Base PCA by Group) ----
plot_and_save_group <- function(key, title_prefix, folder, ext = "svg", point_size = 14) {
    grp <- build_group(key)
    keep <- !is.na(grp)
    ...
    p <- factoextra::fviz_pca_ind(
        X, geom = "point", habillage = grp2, addEllipses = FALSE, ellipse.type = "t", ellipse.level = 0.5,
        palette = pal, repel = TRUE, mean.point = FALSE, title = sprintf("%s by %s", title_prefix, key)
    )
    ...
    p <- p + theme_pca_min() +
        theme(axis.line.x = element_line(color = "#E0E0E0"), axis.line.y = element_line(color = "#E0E0E0")) +
        labs(x = lab_x, y = lab_y, subtitle = NULL, caption = NULL)
    fname <- make_filename("pca", key, ext)
    save_plot(folder, fname, p)
    p
}

# ---- 11. Example Usage ----
if ("celltype" %in% colnames(meta)) plot_and_save_group("celltype", "PCA", folders$plots_base)
if ("region" %in% colnames(meta)) plot_and_save_group("region", "PCA", folders$plots_base)
if ("ExpGroup" %in% colnames(meta)) plot_and_save_group("ExpGroup", "PCA", folders$plots_base)
if ("phenotypeWithinUnit" %in% colnames(meta)) plot_and_save_group("phenotypeWithinUnit", "PCA", folders$plots_base)

# ---- 12. Save Metadata ----
save_table(folders$meta, "sample_metadata_parsed.csv", meta, row.names = TRUE)

# ---- 13. Scree Plot Example ----
var_explained <- (pca$sdev^2) / sum(pca$sdev^2)
df_scree <- data.frame(PC = seq_along(var_explained), Variance = var_explained, Cumulative = cumsum(var_explained))
...
# ---- 15. (Add more analysis/plots as needed, using folders$... for output) ----

# End of improved PCA pipeline with organized folder and naming structure
...
# Read GCT and extract mat/meta
g <- read_gct(gct_file)
mat <- g$mat
meta <- g$meta

# Ensure meta rownames are sample_id and mat colnames are sample_id
if (!"sample_id" %in% colnames(meta)) stop("meta must have a sample_id column")
colnames(mat) <- trim_ws(colnames(mat))
meta$sample_id <- trim_ws(meta$sample_id)
rownames(meta) <- meta$sample_id
...
# Remove samples marked for exclusion if present
if ("exclude" %in% colnames(meta)) {
    keep <- !tolower(meta$exclude) %in% c("true", "1", "yes")
    mat <- mat[, keep, drop = FALSE]
    meta <- meta[keep, , drop = FALSE]
...
# Align names: ensure pca$x rownames and meta rownames are sample_id
rownames(pca$x) <- colnames(mat)
meta <- meta[rownames(pca$x), , drop = FALSE]
stopifnot(identical(rownames(meta), rownames(pca$x)))

# Derived labels: skip, as meta already has all needed columns
...
# Use available grouping columns: celltype, region, ExpGroup, phenotypeWithinUnit
if ("celltype" %in% colnames(meta)) plot_and_save_group("celltype", "PCA", folders$plots_base, ext = "svg")
if ("region" %in% colnames(meta)) plot_and_save_group("region", "PCA", folders$plots_base, ext = "svg")
if ("ExpGroup" %in% colnames(meta)) plot_and_save_group("ExpGroup", "PCA", folders$plots_base, ext = "svg")
if ("phenotypeWithinUnit" %in% colnames(meta)) plot_and_save_group("phenotypeWithinUnit", "PCA", folders$plots_base, ext = "svg")
...
save_table(folders$correlations, "pc_meta_anova.csv", anova_tab)
...
add_pca_ellipses("ExpGroup", "pca_ellipses_ExpGroup.svg")
if ("phenotypeWithinUnit" %in% colnames(meta)) add_pca_ellipses("phenotypeWithinUnit", "pca_ellipses_phenotypeWithinUnit.svg")
...
pc_boxplots_by_group(5, "region")
pc_boxplots_by_group(5, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) pc_boxplots_by_group(5, "phenotypeWithinUnit")
...
pc_violin_plots(5, "region")
pc_violin_plots(5, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) pc_violin_plots(5, "phenotypeWithinUnit")
...
sample_dendrogram(10, "region")
sample_dendrogram(10, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) sample_dendrogram(10, "phenotypeWithinUnit")
...
combined_pca_panels <- function() {
    ...
    for (key in groupings) {
        ...
        p <- ggplot(df, aes(PC1, PC2, color = group)) +
            geom_point(size = 6, alpha = 0.8, stroke = 0) +
            scale_color_manual(values = pal) +
            theme_pca_min() +
            theme(
...
pca_with_ellipses_main("region", 0.95)
pca_with_ellipses_main("ExpGroup", 0.95)
if ("phenotypeWithinUnit" %in% colnames(meta)) pca_with_ellipses_main("phenotypeWithinUnit", 0.95)
...
statistical_summary_barplot("region")
statistical_summary_barplot("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) statistical_summary_barplot("phenotypeWithinUnit")
...
statistical_summary_barplot_umap("region")
statistical_summary_barplot_umap("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) statistical_summary_barplot_umap("phenotypeWithinUnit")
...
pc1_distribution_main("region")
pc1_distribution_main("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) pc1_distribution_main("phenotypeWithinUnit")
...
multipanel_pca_combinations <- function(group_key = "celltype") {
    ...
    p12 <- ggplot(df12, aes(PC1, PC2, color = group)) +
        geom_point(size = 12, alpha = 0.8, stroke = 0) +
        scale_color_manual(values = pal) +
        theme_pca_min() +
        theme(
    ...
    if (ncol(pca$x) >= 3) {
        ...
        if (requireNamespace("gridExtra", quietly = TRUE)) {
            ...
            ggsave(file.path(ensure_dir(subdir("plots/main_figure")), 
                   ...
                   combined, width = 24, height = 6, dpi = 300)
        }
    }
}
multipanel_pca_combinations("celltype")
multipanel_pca_combinations("region")
multipanel_pca_combinations("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) multipanel_pca_combinations("phenotypeWithinUnit")
...
pc_arc_diagram("region", 50)
pc_arc_diagram("ExpGroup", 50)
if ("phenotypeWithinUnit" %in% colnames(meta)) pc_arc_diagram("phenotypeWithinUnit", 50)
...
radial_pca_plot("region")
radial_pca_plot("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) radial_pca_plot("phenotypeWithinUnit")
...
alluvial_pca_flow("region")
alluvial_pca_flow("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) alluvial_pca_flow("phenotypeWithinUnit")
...
ridgeline_pc_distributions(3, "region")
ridgeline_pc_distributions(3, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) ridgeline_pc_distributions(3, "phenotypeWithinUnit")
...
hexbin_pca_with_marginals <- function(group_key = "celltype") {
    ...
    outdir <- ensure_dir(subdir("plots/innovative"))
    out_png <- file.path(outdir, "pca_marginal_densities.png")
    out_svg <- file.path(outdir, "pca_marginal_densities.svg")
    ggsave(out_png, p_marg, width = 5, height = 4, dpi = 300)
    ggsave(out_svg, p_marg, width = 5, height = 4, device = "svg")
}
hexbin_pca_with_marginals("celltype")
hexbin_pca_with_marginals("region")
hexbin_pca_with_marginals("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) hexbin_pca_with_marginals("phenotypeWithinUnit")
...
parallel_coordinates_pcs(5, "region")
parallel_coordinates_pcs(5, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) parallel_coordinates_pcs(5, "phenotypeWithinUnit")
...
variance_sunburst <- function() {
    if (!requireNamespace("treemap", quietly = TRUE) || !requireNamespace("d3r", quietly = TRUE)) {
        ...
        p <- ggplot(df, aes(area = Variance, fill = Category, label = PC)) +
            ...
            scale_fill_manual(values = c("#9d8dc4", "#ccc6ad", "#f0974f")) +
            theme_minimal() +
            labs(title = "Variance Contribution Treemap")
        
        save_plot("plots/innovative", "variance_treemap.svg", p, width = 5, height = 4)
        return(NULL)
    }
}
variance_sunburst()
...
chord_diagram_groups("region")
chord_diagram_groups("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) chord_diagram_groups("phenotypeWithinUnit")
...
sample_trajectory_multiples(16, "region")
sample_trajectory_multiples(16, "ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) sample_trajectory_multiples(16, "phenotypeWithinUnit")
...
hexbin_umap_with_marginals <- function(group_key = "celltype") {
    ...
    ggsave(file.path(ensure_dir(subdir("plots/umap")), 
           ...
           p_marg, width = 5, height = 4, dpi = 300)
}
hexbin_umap_with_marginals("celltype")
hexbin_umap_with_marginals("region")
hexbin_umap_with_marginals("ExpGroup")
if ("phenotypeWithinUnit" %in% colnames(meta)) hexbin_umap_with_marginals("phenotypeWithinUnit")
