...
# Candidate Figure 2 entry point.
#
# THIS IS NOT figures/figure_02.R AND DOES NOT TOUCH IT. The canonical
...
Sys.setenv(PROTEOMICS_SCRIPT_ID = "figures/candidate_figure_02.R")
a <- cf_args()
res <- cf_build_figure("02", check_only = a$check_only, dry_run = a$dry_run)
if (!a$check_only && !a$dry_run) {
  ...
  cat("\nCANDIDATE LAYER ONLY. Canonical Figure 2 is unchanged.\n")
}
