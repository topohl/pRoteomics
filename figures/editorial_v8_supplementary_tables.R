...
Sys.setenv(PROTEOMICS_SCRIPT_ID = "figures/editorial_v8_supplementary_tables.R")

args <- commandArgs(trailingOnly = TRUE)
if ("--dry-run" %in% args || is_dry_run()) {
  message("[DRY-RUN] editorial_v8 supplementary statistics tables")
  quit(save = "no", status = 0L)
}

SD <- path_results("source_data", "manuscript_candidates", "editorial_v8")
OUT <- path_results("tables", "manuscript_candidates", "editorial_v8",
                    "supplementary")
dir_create(OUT)
...
# ---------------------------------------------------------------- ST1
b <- sidecar("extended_data", "v8_ed_bilateral_full")
...
# ---------------------------------------------------------------- ST2
ex <- sidecar("extended_data", "v8_ed_external_full")
...
ig <- sidecar("extended_data", "v8_ed_internal_full")
...
# ---------------------------------------------------------------- ST4
dp <- sidecar("figure_03", "v8_dap_track")
...
# ---------------------------------------------------------------- ST5
se <- sidecar("extended_data", "v8_ed_ca2_sensitivity")
...
# ---------------------------------------------------------------- ST6
wg <- sidecar("extended_data", "v8_ed_wgcna_phenotype")
