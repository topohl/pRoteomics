...
Sys.setenv(PROTEOMICS_SCRIPT_ID = "figures/editorial_v8_vector_audit.R")
...
REP <- function(...) {
  d <- path_results("reports", "manuscript_candidates", "editorial_v8")
  dir_create(d)
  file.path(d, ...)
}
...
# ---------------------------------------------------------------- story map
story <- c(
"# Final figure story (editorial v8)",
"",
"## Figure 2",
"",
...
writeLines(story, REP("final_figure_story_v8.md"))
cat("written:", relative_to(REP("final_figure_story_v8.md")), "\n")
