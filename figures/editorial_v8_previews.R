...
Sys.setenv(PROTEOMICS_SCRIPT_ID = "figures/editorial_v8_previews.R")
...
ROOT <- path_results("figures", "manuscript_candidates", "editorial_v8")
...
cs <- file.path(PREV, "editorial_v8_contact_sheet.png")
image_write(image_montage(image_join(sheet), tile = "3x", geometry = "640x640+8+8",
                          bg = "white"), cs, format = "png")

out <- do.call(rbind, rows)
tab <- path_results("tables", "manuscript_candidates", "editorial_v8")
dir_create(tab)
