...
# Those Part-17 files are not modified; only new s9f_* functions are added here.
# =====================================================================
...
s9f_output_paths <- function(figure_key, output_root = path_results()) {
  list(
    panels = file.path(output_root, "figures", "manuscript_candidates", "final_truth_v9",
                       figure_key, "panels"),
    assembled = file.path(output_root, "figures", "manuscript_candidates", "final_truth_v9",
                          figure_key, "assembled"),
    source_data = file.path(output_root, "source_data", "manuscript_candidates",
                            "final_truth_v9", figure_key),
    reports = file.path(output_root, "reports", "manuscript_candidates", "final_truth_v9",
                        figure_key)
  )
}

s9f_shared_paths <- function(output_root = path_results()) {
  list(
    figures = file.path(output_root, "figures", "manuscript_candidates", "final_truth_v9"),
    tables = file.path(output_root, "tables", "manuscript_candidates", "final_truth_v9"),
    reports = file.path(output_root, "reports", "manuscript_candidates", "final_truth_v9")
  )
}
...
s9f_renderer_sources <- function() {
  c(repo_path("R", "final_truth_v9_panels.R"),
    ...
    repo_path("figures", "final_truth_v9_figure_02.R"),
    repo_path("figures", "final_truth_v9_figure_03.R"),
    repo_path("figures", "final_truth_v9_extended_data.R"))
}

s9f_contract <- function(path = s9f_contract_path()) {
  y <- yaml::read_yaml(path)
  ids <- vapply(y$panels, function(p) as.character(p$id), character(1))
  if (anyDuplicated(ids)) {
    stop("duplicate final_truth_v9 panel id(s): ",
         paste(unique(ids[duplicated(ids)]), collapse = ", "), call. = FALSE)
  }
  if (any(grepl("^[23][a-f]$", ids))) {
    stop("final_truth_v9 panel id collides with the canonical namespace", call. = FALSE)
  }
  for (f in y$figures) {
...
s9f_panel_by_id <- function(contract, id) {
  hit <- Filter(function(p) identical(as.character(p$id), as.character(id)),
                contract$panels)
  if (!length(hit)) stop("final_truth_v9 panel not declared: ", id, call. = FALSE)
  hit[[1]]
}
...
s9f_build <- function(figure_key, dry_run = FALSE) {
  ...
  paths <- s9f_output_paths(figure_key)
  ...
  if (dry_run) {
    ...
    message("[DRY-RUN] final_truth_v9 candidate layer; canonical, Part-16 and Part-17 untouched.")
  ...
  write_csv_safe(panel_records, file.path(paths$reports, "final_truth_v9_panel_status.csv"))
  write_csv_safe(assembly_records, file.path(paths$reports, "final_truth_v9_variant_status.csv"))
  qa <- s9f_qa(panel_records, contract, figure_key)
  qa_path <- file.path(paths$reports, "final_truth_v9_qa.csv")
...
s9f_qa <- function(records, contract, figure_key) {
  fails <- character(0)
  ...
  for (i in seq_len(nrow(records))) {
    fs <- s9f_font_sizes(repo_path(records$svg[i]))
