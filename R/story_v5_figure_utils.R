...
# Those Part-17 files are not modified; only new s5e_* functions are added here.
# =====================================================================
...
s5e_contract_path <- function() repo_path("figures", "figure_story_v5_contract.yml")
s5e_contract_version <- function() "manuscript_story_v5_figures_v1"

s5e_output_paths <- function(figure_key, output_root = path_results()) {
  list(
    panels = file.path(output_root, "figures", "manuscript_candidates", "story_v5",
                       figure_key, "panels"),
    assembled = file.path(output_root, "figures", "manuscript_candidates", "story_v5",
                          figure_key, "assembled"),
    source_data = file.path(output_root, "source_data", "manuscript_candidates",
                            "story_v5", figure_key),
    reports = file.path(output_root, "reports", "manuscript_candidates", "story_v5",
                        figure_key)
  )
}

s5e_shared_paths <- function(output_root = path_results()) {
  list(
    figures = file.path(output_root, "figures", "manuscript_candidates", "story_v5"),
    tables = file.path(output_root, "tables", "manuscript_candidates", "story_v5"),
    reports = file.path(output_root, "reports", "manuscript_candidates", "story_v5")
  )
}

s5e_renderer_sources <- function() {
  c(repo_path("R", "story_v5_figure_panels.R"),
    repo_path("R", "story_v5_figure_utils.R"),
    repo_path("figures", "story_v5_figure_02.R"),
    repo_path("figures", "story_v5_figure_03.R"),
    repo_path("figures", "story_v5_extended_data.R"),
    repo_path("figures", "story_v5_contact_sheet.R"))
}

s5e_contract <- function(path = s5e_contract_path()) {
  y <- yaml::read_yaml(path)
  ids <- vapply(y$panels, function(p) as.character(p$id), character(1))
  if (anyDuplicated(ids)) {
    stop("duplicate story_v5 panel id(s): ",
         paste(unique(ids[duplicated(ids)]), collapse = ", "), call. = FALSE)
  }
  if (any(grepl("^[23][a-f]$", ids))) {
    stop("story_v5 panel id collides with the canonical namespace", call. = FALSE)
  }
  for (f in y$figures) {
...
s5e_panel_by_id <- function(contract, id) {
  hit <- Filter(function(p) identical(as.character(p$id), as.character(id)),
                contract$panels)
  if (!length(hit)) stop("story_v5 panel not declared: ", id, call. = FALSE)
  hit[[1]]
}

s5e_build <- function(figure_key, dry_run = FALSE) {
  contract <- s5e_contract()
  nv_assert_no_model_fitting(s5e_renderer_sources())
  figs <- Filter(function(f) identical(as.character(f$figure_key), figure_key),
                 contract$figures)
  if (!length(figs)) stop("no story_v5 figure with key: ", figure_key, call. = FALSE)
  ...
  paths <- s5e_output_paths(figure_key)
  ...
  if (dry_run) {
    ...
    message("[DRY-RUN] story_v5 candidate layer; canonical, Part-16 and Part-17 untouched.")
  ...
  if (length(stale)) {
    message("[story_v5] removing ", length(stale), " stale panel file(s): ",
  ...
  write_csv_safe(panel_records, file.path(paths$reports, "story_v5_panel_status.csv"))
  write_csv_safe(assembly_records, file.path(paths$reports, "story_v5_variant_status.csv"))
