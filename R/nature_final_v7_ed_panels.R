...
nf_ed_bilateral_full <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
...
nf_ed_external_full <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
...
# ==========================================================================
nf_ed_internal_full <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
...
nf_ed_wgcna_phenotype <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  ...
  pick <- function(cands, what) {
    ...
    stop("nf_ed_wgcna_phenotype: no populated ", what, " column", call. = FALSE)
  }
  ...
  p <- ggplot2::ggplot(z, ggplot2::aes(xpos, ypos)) +
    ...
    ggplot2::labs(x = NULL, y = NULL,
                  caption = sprintf(paste0(
                    "DESCRIPTIVE ONLY. %d of %d modules show RES-CON > 0, ",
                    "SUS-CON < 0 and SUS-RES < 0, but 0 of 45 module x contrast ",
                    "cells reach tier-specific FDR support.\nThe colour scale ",
                    "must not be read as evidence of a group difference."),
                    geom_n, nrow(w))) +
    nf_theme_tile() +
  ...
  p <- p +
    ...
    ggplot2::annotate("text", x = length(cl) + 0.8, y = ny + 0.35, hjust = 0,
                      family = fam, size = nf_sz(5.0), fontface = "bold",
                      colour = "grey20", label = "external cell type")
  z$inferential_status <- sprintf(
    "%d of %d modules show the descriptive geometry; 0 of 45 cells FDR-supported",
    geom_n, nrow(w))
...
nf_ed_m11 <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  ...
  p <- ggplot2::ggplot(z, ggplot2::aes(xpos, mean_con_z)) +
    ggplot2::geom_hline(yintercept = 0, linewidth = nv_lw("reference_pt"),
                        colour = "grey70") +
    ggplot2::geom_col(fill = "#7A9BB0", width = 0.72) +
    ggplot2::scale_x_continuous(breaks = seq_len(nrow(u)), labels = u$display,
                                expand = c(0, 0.6)) +
    ggplot2::labs(x = NULL, y = "mean CON z",
                  title = paste0("neuropil ", lab, "  -  worked example"),
                  caption = paste0(
                    "spatial peak ", peak, "; external cell-type affinity ",
                    ifelse(is.na(cell), "not assigned", cell),
                    "; bilateral ", ifelse(is.na(bil), "not assigned", bil),
                    ".\nThe active historical label is unchanged and the ",
                    "proposed oligodendrocyte / myelin label remains ",
                    "unactivated; the external evidence is shown as context ",
                    "only.")) +
    nf_theme(grid = "y") +
    ggplot2::theme(axis.text.x = ggplot2::element_text(size = NF_MIN_PT,
                                                       angle = 45, hjust = 1),
...
# ==========================================================================
# ED8: similarity matrices with CONTROLLED geometry
# ==========================================================================
#
# Part 22 found free facet scales gave the 4x4 soma and microglia matrices the
# same width as the 10x10 neuropil matrix, so each soma cell was about three
# times the linear size of a neuropil cell. Here facet widths are proportional
# to unit count, and the not-connectivity statement is promoted from a small
# caption into the panel title.
nf_ed_similarity <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  ...
  p <- ggplot2::ggplot(sym, ggplot2::aes(xf, yf)) +
    ...
    ggplot2::scale_x_discrete(labels = function(x) unname(disp[as.character(x)]),
                              drop = TRUE) +
    ggplot2::scale_y_discrete(labels = function(x) unname(disp[as.character(x)]),
                              drop = TRUE) +
    # space = "free" makes facet width proportional to the number of units, so a
    # 4x4 cell is the same physical size as a 10x10 cell
    ggplot2::facet_grid(. ~ comp, scales = "free_x", space = "free_x") +
    ggplot2::labs(
      x = NULL, y = NULL,
      title = "Molecular similarity between spatial proteomic profiles - NOT anatomical connectivity",
      caption = paste0(
        "A value is the correlation between two units' protein profiles, not a ",
        "projection. CON animals only (n = 3); stored median across CON ",
        "animals. Diagonal omitted.")) +
    nf_theme_tile() +
    ggplot2::theme(
      axis.text.x = ggplot2::element_text(size = NF_MIN_PT, angle = 90,
                                          hjust = 1, vjust = 0.5,
                                          colour = "grey30"),
      axis.text.y = ggplot2::element_text(size = NF_MIN_PT, colour = "grey30"),
      strip.text = ggplot2::element_text(size = nf_pt(5.4), face = "bold"),
      plot.title = ggplot2::element_text(size = nf_pt(5.6), face = "bold",
                                         colour = "grey10"),
      legend.position = "right",
      legend.key.width = ggplot2::unit(1.6, "mm"),
      legend.key.height = ggplot2::unit(3.6, "mm"),
      panel.spacing.x = ggplot2::unit(2.4, "mm"))
  out <- sym[, c("dataset", "r", "c", "v")]
  names(out) <- c("dataset", "unit_row", "unit_col", "median_similarity")
  out$reading <- "similarity between spatial proteomic profiles, NOT anatomical connectivity"
...
nf_ed_fingerprint_full <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  z <- nv_read_csv(repo_path(panel$primary_source))
...
nf_ed_module_fingerprint <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  z <- s6_module_fingerprint_table()
...
nf_ed_locations <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  h <- s6_ed7_qualified()
  ...
  p <- ggplot2::ggplot(z) +
    ...
    ggplot2::labs(x = NULL, y = NULL,
                  caption = sprintf(paste0(
                    ...
                    "excluded.\nRed = strongest effect outside the baseline ",
                    "dominant unit."),
                    nrow(z), n_all,
...
# ---- CA2-SLM locator ------------------------------------------------------
nf_ed_ca2_locator <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
