...
# Those Part-17 files are not modified; only new sv_* functions are added here.
# =====================================================================
...
sv_contract_path <- function() repo_path("figures", "figure_story_v3_contract.yml")
sv_contract_version <- function() "manuscript_story_v3_figures_v1"

sv_output_paths <- function(figure_key, output_root = path_results()) {
  list(
    panels = file.path(output_root, "figures", "manuscript_candidates", "story_v3",
                       figure_key, "panels"),
    assembled = file.path(output_root, "figures", "manuscript_candidates", "story_v3",
                          figure_key, "assembled"),
    source_data = file.path(output_root, "source_data", "manuscript_candidates",
                            "story_v3", figure_key),
    reports = file.path(output_root, "reports", "manuscript_candidates", "story_v3",
                        figure_key)
  )
}

sv_shared_paths <- function(output_root = path_results()) {
  list(
    figures = file.path(output_root, "figures", "manuscript_candidates", "story_v3"),
    tables = file.path(output_root, "tables", "manuscript_candidates", "story_v3"),
    reports = file.path(output_root, "reports", "manuscript_candidates", "story_v3")
  )
}

sv_renderer_sources <- function() {
  c(repo_path("R", "story_v3_figure_panels.R"),
    repo_path("R", "story_v3_figure_utils.R"),
    repo_path("figures", "story_v3_figure_02.R"),
    repo_path("figures", "story_v3_figure_03.R"),
    repo_path("figures", "story_v3_extended_data.R"),
    repo_path("figures", "story_v3_contact_sheet.R"))
}

sv_contract <- function(path = sv_contract_path()) {
  y <- yaml::read_yaml(path)
  ids <- vapply(y$panels, function(p) as.character(p$id), character(1))
  if (anyDuplicated(ids)) {
    stop("duplicate story_v3 panel id(s): ",
         paste(unique(ids[duplicated(ids)]), collapse = ", "), call. = FALSE)
  }
  if (any(grepl("^[23][a-f]$", ids))) {
    stop("story_v3 panel id collides with the canonical namespace", call. = FALSE)
  }
  for (f in y$figures) {
...
sv_panel_by_id <- function(contract, id) {
  hit <- Filter(function(p) identical(as.character(p$id), as.character(id)),
                contract$panels)
  if (!length(hit)) stop("story_v3 panel not declared: ", id, call. = FALSE)
  hit[[1]]
}

sv_build <- function(figure_key, dry_run = FALSE) {
  contract <- sv_contract()
  nv_assert_no_model_fitting(sv_renderer_sources())
  figs <- Filter(function(f) identical(as.character(f$figure_key), figure_key),
                 contract$figures)
  if (!length(figs)) stop("no story_v3 figure with key: ", figure_key, call. = FALSE)
  ...
  paths <- sv_output_paths(figure_key)
  ...
  if (dry_run) {
    ...
    message("[DRY-RUN] story_v3 candidate layer; canonical, Part-16 and Part-17 untouched.")
  ...
  write_csv_safe(panel_records, file.path(paths$reports, "story_v3_panel_status.csv"))
  write_csv_safe(assembly_records, file.path(paths$reports, "story_v3_variant_status.csv"))
