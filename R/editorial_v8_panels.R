...
e8_schematic <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
...
e8_external_main <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  ...
  k$lab <- paste0(e8_short_contrast(k$internal_contrast), "  →  ",
...
e8_internal_main <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  ...
  g$lab <- paste0(e8_short_contrast(g$contrast), "  →  ", g$Description)
...
e8_dap_track <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  z <- nf_dap_counts()
...
e8_prot_values <- function(program_key, top_n = 7L) {
  pr <- s4_programs()
  prog <- pr[pr$key == program_key, , drop = FALSE]
  if (!nrow(prog)) stop("e8_prot_values: unknown program key: ", program_key,
                        call. = FALSE)
...
e8_gsea_curve <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  pr <- s4_programs()
  prog <- pr[pr$key == as.character(panel$program_key), , drop = FALSE]
  if (!nrow(prog)) stop("e8_gsea_curve: unknown program key", call. = FALSE)
  ev <- s4_gsea_scores(prog)
  acc <- prog$accent[1]
  N <- ev$N
...
e8_anatomy_bridge <- function(panel, svg_path, csv_path, w_mm, h_mm) {
  fam <- nf_fam()
  dcol <- nv_dataset_colours()
  rows <- nf_bridge_rows()
  ex <- s6_exemplars()
...
e8_atlas_cells <- function(th, contrast) {
