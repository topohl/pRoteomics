...
# Those Part-17 files are not modified; only new s8e_* functions are added here.
# =====================================================================
...
s8e_contract_path <- function() repo_path("figures", "figure_editorial_v8_contract.yml")
s8e_contract_version <- function() "manuscript_editorial_v8_figures_v1"

s8e_output_paths <- function(figure_key, output_root = path_results()) {
  list(
    panels = file.path(output_root, "figures", "manuscript_candidates", "editorial_v8",
                       figure_key, "panels"),
    assembled = file.path(output_root, "figures", "manuscript_candidates", "editorial_v8",
                          figure_key, "assembled"),
    source_data = file.path(output_root, "source_data", "manuscript_candidates",
                            "editorial_v8", figure_key),
    reports = file.path(output_root, "reports", "manuscript_candidates", "editorial_v8",
                        figure_key)
  )
}

s8e_shared_paths <- function(output_root = path_results()) {
  list(
    figures = file.path(output_root, "figures", "manuscript_candidates", "editorial_v8"),
    tables = file.path(output_root, "tables", "manuscript_candidates", "editorial_v8"),
    reports = file.path(output_root, "reports", "manuscript_candidates", "editorial_v8")
  )
}

s8e_renderer_sources <- function() {
  c(repo_path("R", "editorial_v8_panels.R"),
    ...
    repo_path("figures", "editorial_v8_figure_02.R"),
    repo_path("figures", "editorial_v8_figure_03.R"),
    repo_path("figures", "editorial_v8_extended_data.R"))
}

s8e_contract <- function(path = s8e_contract_path()) {
  y <- yaml::read_yaml(path)
  ids <- vapply(y$panels, function(p) as.character(p$id), character(1))
  if (anyDuplicated(ids)) {
    stop("duplicate editorial_v8 panel id(s): ",
         paste(unique(ids[duplicated(ids)]), collapse = ", "), call. = FALSE)
  }
  if (any(grepl("^[23][a-f]$", ids))) {
    stop("editorial_v8 panel id collides with the canonical namespace", call. = FALSE)
  }
  for (f in y$figures) {
...
s8e_panel_by_id <- function(contract, id) {
  hit <- Filter(function(p) identical(as.character(p$id), as.character(id)),
                contract$panels)
  if (!length(hit)) stop("editorial_v8 panel not declared: ", id, call. = FALSE)
  hit[[1]]
}

s8e_build <- function(figure_key, dry_run = FALSE) {
  contract <- s8e_contract()
  nv_assert_no_model_fitting(s8e_renderer_sources())
  figs <- Filter(function(f) identical(as.character(f$figure_key), figure_key),
                 contract$figures)
  if (!length(figs)) stop("no editorial_v8 figure with key: ", figure_key, call. = FALSE)
  ...
  paths <- s8e_output_paths(figure_key)
  ...
  if (dry_run) {
    ...
    message("[DRY-RUN] editorial_v8 candidate layer; canonical, Part-16 and Part-17 untouched.")
  ...
  if (length(stale)) {
    message("[editorial_v8] removing ", length(stale), " stale panel file(s): ",
            paste(stale, collapse = ", "))
    file.remove(file.path(paths$panels, stale))
  }
  e8_capture_on(); e8_registry_clear()
  on.exit(e8_capture_off(), add = TRUE)
  ...
  for (f in figs) {
    ...
    # directly from the captured ggplot grobs, so the SVG and the PDF are the
    # same drawing and the PDF is genuine vector.
    svg_names <- lapply(panel_paths, basename)
    e8_compose_svg(f, svg_names, target)
    pdf <- sub("[.]svg$", ".pdf", target)
    e8_compose_pdf(f, svg_names, pdf)
  ...
  write_csv_safe(panel_records, file.path(paths$reports, "editorial_v8_panel_status.csv"))
  write_csv_safe(assembly_records, file.path(paths$reports, "editorial_v8_variant_status.csv"))
  qa <- s8e_qa(panel_records, contract, figure_key)
  qa_path <- file.path(paths$reports, "editorial_v8_qa.csv")
  write_csv_safe(data.frame(
    ...
    qa_path)
  if (length(qa)) {
    stop("editorial_v8 QA FAILED:
  ", paste(qa, collapse = "
  "),
         call. = FALSE)
  }
  message("[editorial_v8] QA passed for ", figure_key)
...
s8e_qa <- function(records, contract, figure_key) {
  fails <- character(0)
  ...
  for (i in seq_len(nrow(records))) {
    fs <- s8e_font_sizes(repo_path(records$svg[i]))
