...
load_wgcna_marker_sets <- function(include_empirical = TRUE, include_legacy_aliases = TRUE, quiet = FALSE) {
  ...
  if (!length(sets)) {
    if (!isTRUE(quiet)) warning("Falling back to legacy hard-coded WGCNA marker panels; run analysis/qc/build_reference_marker_registry.R to create the registry.", call. = FALSE)
...
empty_group_effects <- function(dataset, level, reason) {
  data.frame(
    ...
    p_value = NA_real_, FDR_within_dataset_level = NA_real_,
    FDR_dataset_all_levels = NA_real_, FDR_global = NA_real_,
    FDR_family_within_level_id = NA_character_,
    FDR_family_dataset_id = NA_character_,
    n_tests_FDR_within_dataset_level = NA_integer_,
    n_tests_FDR_dataset_all_levels = NA_integer_,
    ...
    min_unique_animals_compared_group = NA_integer_,
    n_samples_per_spatial_unit = NA_character_,
    n_animals_per_spatial_unit = NA_character_,
    fixed_effect_rank = NA_integer_, fixed_effect_columns = NA_integer_,
    convergence_status = NA_character_, convergence_warnings = NA_character_,
    optimizer_messages = NA_character_, optimizer_code = NA_character_,
    residual_df = NA_real_, AnimalID_source = NA_character_,
    animal_id_mapping_status = NA_character_,
    ...
    endpoint_construction_method = NA_character_,
    endpoint_provenance_status = NA_character_,
...
required_group_effect_columns <- c(
  ...
  "min_unique_animals_compared_group",
  "n_samples_total", "n_samples_per_group", "animal_level_status", "pseudoreplication_guard",
  "n_samples_per_spatial_unit", "n_animals_per_spatial_unit",
  "contrast", "estimate", "SE", "statistic", "p_value",
  "FDR_within_dataset_level", "FDR_dataset_all_levels", "FDR_global",
  "FDR_family_within_level_id", "FDR_family_dataset_id",
  "n_tests_FDR_within_dataset_level", "n_tests_FDR_dataset_all_levels",
  "FDR_method", "evidence_status", "direction", "n_samples",
  ...
  "fixed_effect_rank", "fixed_effect_columns", "convergence_status",
  "convergence_warnings", "optimizer_messages", "optimizer_code", "residual_df",
  "AnimalID_source", "animal_id_mapping_status", "membership_version",
  "identity_contract_version", "identity_contract_sha256",
  "frozen_state_sha256", "endpoint_construction_method",
