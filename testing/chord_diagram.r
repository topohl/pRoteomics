...
# =========================================================
# Proteomics overlaps: plots + Excel shared-protein exports
# + Internal-uniques chord (shared vs unique within sector)
# =========================================================

# Packages
pkgs <- c("dplyr","tidyr","circlize","RColorBrewer","tools","magick","grid","openxlsx","digest","tibble")
to_install <- setdiff(pkgs, rownames(installed.packages()))
if (length(to_install)) install.packages(to_install, quiet = TRUE)
invisible(lapply(pkgs, library, character.only = TRUE))
...
# -------- Original plotting helper (shared-only) --------
plot_chord <- function(edge_df, title_text) {
  if (nrow(edge_df) == 0) {
    grid::grid.newpage()
    grid::grid.text(paste0("No edges: ", title_text))
...
# -------- Save shared-only plots (original outputs) --------
f_up   <- file.path(out_dir, "chord_upregulated.png")
f_down <- file.path(out_dir, "chord_downregulated.png")
f_side <- file.path(out_dir, "chord_up_down_side_by_side.png")
...
# -------- Excel exports: shared proteins per pair (original) --------
if (!requireNamespace("openxlsx", quietly = TRUE)) install.packages("openxlsx")
library(openxlsx)
...
# =========================================================
# Internal uniques chord: draw unique proteins as internal chords
# =========================================================

# Direction-specific unique counts per comparison
unique_counts_dir <- function(dir_sign = +1, lfc_min = 0) {
  is_sig <- sig
  for (cc in comp_cols) {
    vv <- wide_fc[[cc]]
    is_sig[[cc]] <- is_sig[[cc]] & (sign(vv) == dir_sign) & (abs(vv) >= lfc_min)
  }
  res <- lapply(comp_cols, function(cc) {
    this <- is_sig[[cc]]
    others <- Reduce(`|`, is_sig[setdiff(comp_cols, cc)], init = rep(FALSE, nrow(is_sig)))
    uniq_idx <- which(this & !others)
    tibble::tibble(comparison = cc, n_unique = length(uniq_idx))
  })
  dplyr::bind_rows(res)
}

unique_up   <- unique_counts_dir(+1, lfc_min = 0)
unique_down <- unique_counts_dir(-1, lfc_min = 0)

# Robust helpers for sector sizing
sv <- function(v, nm) {
  out <- v[[nm]]
  if (is.null(out) || is.na(out)) return(0)
  out
}

shared_totals <- function(edge_df) {
  all_nodes <- comp_cols
  if (nrow(edge_df) == 0) return(setNames(rep(0L, length(all_nodes)), all_nodes))
  w_from <- tapply(edge_df$weight, edge_df$from, sum)
  w_to   <- tapply(edge_df$weight, edge_df$to,   sum)
  res <- sapply(all_nodes, function(nm) sv(w_from, nm) + sv(w_to, nm))
  setNames(as.numeric(res), all_nodes)
}

tot_up_shared   <- shared_totals(edges_up)
tot_down_shared <- shared_totals(edges_down)

make_sectors <- function(tot_shared, unique_df) {
  uvec <- setNames(unique_df$n_unique, unique_df$comparison)
  all_nodes <- comp_cols
  ucount <- function(nm) { val <- uvec[[nm]]; if (is.null(val) || is.na(val)) 0 else val }
  data.frame(
    sector = c(paste0(all_nodes,"_shared"), paste0(all_nodes,"_unique")),
    x      = c(unname(tot_shared[all_nodes]),
               unname(sapply(all_nodes, ucount))),
    base   = rep(all_nodes, 2),
    part   = rep(c("shared","unique"), each = length(all_nodes)),
    stringsAsFactors = FALSE
  )
}

sectors_up   <- make_sectors(tot_up_shared,   unique_up)
sectors_down <- make_sectors(tot_down_shared, unique_down)

make_links <- function(edge_df_shared, unique_df) {
  Ls <- if (nrow(edge_df_shared)) {
    transform(edge_df_shared,
              from = paste0(from, "_shared"),
              to   = paste0(to,   "_shared"))
  } else edge_df_shared
  Lu <- if (nrow(unique_df)) {
    out <- data.frame(
      from = paste0(unique_df$comparison, "_unique"),
      to   = paste0(unique_df$comparison, "_shared"),
      weight = unique_df$n_unique,
      stringsAsFactors = FALSE
    )
    out[out$weight > 0, , drop = FALSE]
  } else data.frame(from=character(),to=character(),weight=integer())
  dplyr::bind_rows(Ls, Lu)
}

links_up   <- make_links(edges_up,   unique_up)
links_down <- make_links(edges_down, unique_down)

plot_chord_internal <- function(links, sectors, title_text) {
  sectors <- subset(sectors, x > 0)
  if (nrow(links) == 0 || nrow(sectors) == 0) {
    grid::grid.newpage(); grid::grid.text(paste0("No edges: ", title_text)); return(invisible(NULL))
  }

  bases <- unique(sectors$base)
  pal <- colorRampPalette(RColorBrewer::brewer.pal(8, "Set2"))(length(bases))
  base_col <- setNames(pal, bases)
  grid.col <- setNames(base_col[sectors$base], sectors$sector)

  # Ensure shared and unique sub-sectors of a base are adjacent
  ord <- as.vector(rbind(paste0(bases, "_shared"), paste0(bases, "_unique")))
  ord <- ord[ord %in% sectors$sector]

  circlize::circos.clear()
  circlize::circos.par(start.degree = 90,
                       gap.after = c(rep(2, length(ord)-1), 10),
                       track.margin = c(0.01, 0.01),
                       points.overflow.warning = FALSE)
  ...
  circlize::circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
    si <- circlize::get.cell.meta.data("sector.index")
    if (grepl("_shared$", si)) {
      lab <- sub("_(shared|unique)$","", si)
      circlize::circos.text(circlize::CELL_META$xcenter,
                            circlize::CELL_META$ylim[1] + circlize::mm_y(3),
                            lab, facing = "clockwise", niceFacing = TRUE,
                            adj = c(0, 0.5), cex = 0.9, col = "#222")
    }
  }, bg.border = NA)
...
# -------- Draw internal-uniques outputs --------
f_up_int_svg <- file.path(out_dir, "chord_upregulated_internal_uniques.svg")
f_up_int_png <- file.path(out_dir, "chord_upregulated_internal_uniques.png")
f_dn_int_svg <- file.path(out_dir, "chord_downregulated_internal_uniques.svg")
f_dn_int_png <- file.path(out_dir, "chord_downregulated_internal_uniques.png")

svg(f_up_int_svg, width = inch_w, height = inch_h)
plot_chord_internal(links_up, sectors_up, "Upregulated: internal uniques")
dev.off()

png(f_up_int_png, width = 2200, height = 2200, res = 250)
plot_chord_internal(links_up, sectors_up, "Upregulated: internal uniques")
dev.off()

svg(f_dn_int_svg, width = inch_w, height = inch_h)
plot_chord_internal(links_down, sectors_down, "Downregulated: internal uniques")
dev.off()

png(f_dn_int_png, width = 2200, height = 2200, res = 250)
plot_chord_internal(links_down, sectors_down, "Downregulated: internal uniques")
dev.off()
