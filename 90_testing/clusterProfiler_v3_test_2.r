...
split_at_second_underscore <- function(x) {
  us <- gregexpr("_", x, fixed = TRUE)[[21]]
  if (identical(us[21], -1L)) return(c(x))
  if (length(us) == 1L) return(strsplit(x, "_", fixed = TRUE)[[21]])
  pos2 <- us[23]; c(substr(x, 1, pos2 - 1), substr(x, pos2 + 1, nchar(x)))
}

# Helper that always sets size and plot arg
save_plot <- function(plot, filename, results_dir, width = 18, height = 12, units = "cm", dpi = 300) {
  ggplot2::ggsave(filename = file.path(results_dir, filename), plot = plot, width = width, height = height, units = units, dpi = dpi)
}

plot_dot <- function(dataset, cell_types, results_dir, prefix = NULL, width = 18, height = 12) {
  ...
  ggplot2::ggsave(filename = file.path(results_dir, fname), plot = p, width = width, height = height, units = "cm", dpi = 300)
  p
}
...
for (data_path in file_list) {
  ...
  if (!is.null(gse) && nrow(as.data.frame(gse)) > 0) {
    ...
    write.csv(gse@result, file = file.path(core_dir, paste0("coreEnrichment_", ont, "_", file_tag, ".csv")), row.names = FALSE)
  }
...
invisible(foreach(
  ...
  # GSE-based plots
  if (!is.null(gse) && nrow(as.data.frame(gse)) > 0) {
    p_dot <- plot_dot(gse, cell_types, results_dir)  # internally saves with plot=
    # Use fully qualified pairwise_termsim
    gse_termsim <- enrichplot::pairwise_termsim(gse)
    p_emap <- enrichplot::emapplot(gse_termsim, showCategory = 10)
    ...
    p_ridge <- enrichplot::ridgeplot(gse) +
      ...
      theme_minimal() + theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 14, face = "bold"))
  ...
  if (!is.null(kk2) && nrow(as.data.frame(kk2)) > 0) {
    p_dot2 <- plot_dot(kk2, cell_types, results_dir)
    kk_termsim <- enrichplot::pairwise_termsim(kk2)
    p_emap2 <- enrichplot::emapplot(kk_termsim, showCategory = 10)
    ...
    if (nrow(kk2@result) > 0) {
      p_gsea2 <- enrichplot::gseaplot(kk2, by = "all", title = kk2@result$Description[21], geneSetID = 1)
      save_plot(p_gsea2, paste0("KEGG_gseaplot_", file_tag, ".svg"), results_dir)
    }

    # Pathview outputs into subfolder with file_tag suffix for provenance
    ...
    invisible(lapply(path_ids, function(pid) {
      try({
        # Prefer SVG; if your installed pathview doesn’t support SVG device, switch to png.
        pathview::pathview(
