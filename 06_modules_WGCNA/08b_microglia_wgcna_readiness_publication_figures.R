...
values <- read_csv(paths$values) |> dplyr::mutate(supermodule_id = as.character(.data$supermodule_id), roi = factor(toupper(.data$SpatialLabel), levels = roi_order), StressGroup = factor(.data$StressGroup, levels = c("CON", "RES", "SUS"))) |> join_labels()
effects <- read_csv(paths$effects) |> dplyr::mutate(supermodule_id = as.character(.data$supermodule_id), estimate = as.numeric(.data$estimate), raw_SE = as.numeric(.data$SE), q_value = as.numeric(.data$FDR_global), stable = .data$primary_model_stable %in% TRUE & !(.data$singular_model %in% TRUE)) |> join_labels()
...
global_source <- values |> dplyr::select(.data$supermodule_id, .data$canonical_plot_label, .data$StressGroup, .data$AnimalID, .data$roi, roi_eigengene = .data$eigengene) |> dplyr::left_join(animal_means, by = c("supermodule_id", "canonical_plot_label", "StressGroup", "AnimalID"), relationship = "many-to-one")
...
global <- effects_std |> dplyr::filter(.data$effect_scope == "spatial_adjusted_global", .data$contrast == "SUS - RES") |> dplyr::mutate(plot_label = factor(.data$canonical_plot_label, levels = rev(labels$canonical_plot_label)), claim_symbol = dplyr::case_when(!.data$stable ~ "none", .data$q_value <= .05 ~ "FDR05", .data$q_value <= .10 ~ "FDR10", TRUE ~ "none"))
...
spatial <- effects_std |> dplyr::filter(.data$effect_scope == "within_spatial_unit", .data$contrast == "SUS - RES") |> dplyr::mutate(roi = factor(toupper(.data$spatial_unit), levels = roi_order), plot_label = factor(.data$canonical_plot_label, levels = rev(labels$canonical_plot_label)), support = dplyr::case_when(!.data$stable ~ "cross", .data$q_value <= .05 ~ "filled", .data$q_value <= .10 ~ "open", TRUE ~ "none"))
